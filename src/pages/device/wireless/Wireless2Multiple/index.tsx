...
export const Wireless2Multiple = () => {
  ...
  const [network, setNetwork] = useState(NetworkType.Network3)
  const [mssid, setMssid] = useState<MSSIDType>(MSSIDType.MSSID1)

  const formik = useFormik<FormikValuesType>({
    initialValues: {
      disabled: result?.[`cbid.wireless.wifi0_mssid${mssid}.disabled`] ?? '1',
      time_schedule:
        result?.[`cbid.wireless.wifi0_mssid${mssid}.time_schedule`] ?? '',
      ssid: result?.[`cbid.wireless.wifi0_mssid${mssid}.ssid`] ?? '',
      mode: result?.[`cbid.wireless.wifi0_mssid${mssid}.mode`] ?? 'ap',
      hidden: result?.[`cbid.wireless.wifi0_mssid${mssid}.hidden`] ?? '0',
      isolate: result?.[`cbid.wireless.wifi0_mssid${mssid}.isolate`] ?? '0',
      encryption:
        result?.[`cbid.wireless.wifi0_mssid${mssid}.encryption`] ?? 'none',
      cipher: result?.[`cbid.wireless.wifi0_mssid${mssid}.cipher`] ?? '',
      _wpa_key: result?.[`cbid.wireless.wifi0_mssid${mssid}._wpa_key`] ?? '',
      auth_server:
        result?.[`cbid.wireless.wifi0_mssid${mssid}.auth_server`] ?? '',
      auth_port:
        result?.[`cbid.wireless.wifi0_mssid${mssid}.auth_port`] ?? '1812',
      auth_secret:
        result?.[`cbid.wireless.wifi0_mssid${mssid}.auth_secret`] ?? '',
    },
    ...
    onSubmit: (values) => {
      const payload = {
        ...
        [`cbid.wireless.wifi0_mssid${mssid}.disabled`]: values.disabled,
        [`cbid.wireless.wifi0_mssid${mssid}.time_schedule`]:
          values.time_schedule,
        [`cbid.wireless.wifi0_mssid${mssid}.ssid`]: values.ssid,
        [`cbid.wireless.wifi0_mssid${mssid}.mode`]: values.mode,
        [`cbid.wireless.wifi0_mssid${mssid}.hidden`]: values.hidden,
        [`cbid.wireless.wifi0_mssid${mssid}.isolate`]: values.isolate,
        [`cbid.wireless.wifi0_mssid${mssid}.encryption`]: values.encryption,
        [`cbid.wireless.wifi0_mssid${mssid}.cipher`]: values.cipher,
        [`cbid.wireless.wifi0_mssid${mssid}._wpa_key`]: values._wpa_key,
        [`cbid.wireless.wifi0_mssid${mssid}.auth_server`]: values.auth_server,
        [`cbid.wireless.wifi0_mssid${mssid}.auth_port`]: values.auth_port,
        [`cbid.wireless.wifi0_mssid${mssid}.auth_secret`]: values.auth_secret,
      }
      // sendWsSetMessage()
      switch (mssid) {
  ...
  const handleMssidOptionChange = (event: SelectChangeEvent<unknown>) => {
  ...
  const encryptionList = optionsConverter(
    options,
    `cbid.wireless.wifi0_mssid${mssid}.encryption`,
  )
  const cipherList = optionsConverter(
    options,
    `cbid.wireless.wifi0_mssid${mssid}.cipher`,
  )
  ...
  return (
    <>
      <PageHeader
        title='Wireless 2.4GHz Multiple SSID'
      ...
      <Stack gap={2}>
        <Card>
          ...
          <CardContent className={classes.fieldWidth}>
            ...
            <Select
              label='Multiple SSID'
              value={`wifi0.network${network}`}
              options={ssidOptions}
              onChange={handleMssidOptionChange}
            />
            <TextField
              {...textfieldProps(
                'ssid',
