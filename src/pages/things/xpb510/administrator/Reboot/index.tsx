...
export const Reboot = () => {
  const data = useSelector(
    (state: RootStateProps) => state.xpb510.administrator.reboot,
  )
  ...
  const scheduleOptions = optionsConverter(
    options,
  ...
  const handleReboot = () => {
    ...
    if (window.confirm('Are you sure you want to restart the device?')) {
      sendWsSetMessage(XPB_EVENT_ACTIONS.XPB_510_REBOOT_PERFORM_REBOOT, payload)
    }
  }
  ...
  const formik = useFormik<FormikValuesType>({
    initialValues: {
      time_schedule:
        result[`cbid.autoreboot.reboot_config.time_schedule`] ?? '',
    },
    onSubmit: (values) => {
      const payload: PayloadType_timeSchedule = {
        [`cbid.autoreboot.reboot_config.time_schedule`]: values.time_schedule,
  ...
  useEffect(() => {
    sendWsGetMessage(XPB_EVENT_ACTIONS.XPB_510_REBOOT_GET_SETTINGS_PAGE)
  }, [sendWsGetMessage])
