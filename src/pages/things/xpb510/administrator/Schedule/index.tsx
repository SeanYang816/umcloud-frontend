...
export const Schedule = () => {
  ...
  const data = useSelector(
    (state: RootStateProps) => state.xpb510.administrator.schedule.schedule,
  )
  ...
  const formik = useFormik<FormikValuesType>({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      sendWsSetMessage(
        XPB_EVENT_ACTIONS.XPB_510_SCHEDULE_ADD_SCHEDULE_RULE,
        payload,
      )
      resetForm()
    },
  ...
  const handleDialogOpen = (key: string) => {
    setEditKey(key)
    sendWsGetMessage(
      XPB_EVENT_ACTIONS.XPB_510_SCHEDULE_GET_SCHEDULE_EDIT_PAGE,
      key,
    )
  }
  ...
  const handleDelete = useCallback(
    (key: string) => {
      ...
      sendWsSetMessage(
        XPB_EVENT_ACTIONS.XPB_510_SCHEDULE_DELETE_SCHEDULE_RULE,
        payload,
      )
    },
    [sendWsSetMessage],
  ...
  const handleApply = useCallback(() => {
    sendWsSetMessage(XPB_EVENT_ACTIONS.XPB_510_SCHEDULE_SET_SCHEDULE_PAGE, {})
  }, [sendWsSetMessage])
  ...
  useEffect(() => {
    setIsFetch(true)
    sendWsGetMessage(XPB_EVENT_ACTIONS.XPB_510_SCHEDULE_GET_SCHEDULE_PAGE)
