...
export const DmzHost = () => {
  ...
  const data = useSelector(
    (state: RootStateProps) => state.xpb510.network.firewall.dmzHost,
  )
  const result = data?.result ?? ({} as DmzHostPageResult)
  ...
  const formik = useFormik<FormikValuesType>({
    ...
    onSubmit: (values) => {
      ...
      sendWsSetMessage(
        XPB_EVENT_ACTIONS.XPB_510_FIREWALL_SET_DMZ_HOST_PAGE,
        payload,
      )
    },
  })
  ...
  useEffect(() => {
    sendWsGetMessage(XPB_EVENT_ACTIONS.XPB_510_FIREWALL_GET_DMZ_HOST_PAGE)
