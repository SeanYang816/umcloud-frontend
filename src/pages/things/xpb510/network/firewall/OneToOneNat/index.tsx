...
export const OneToOneNat = () => {
  ...
  const data = useSelector(
    (state: RootStateProps) => state.xpb510.network.firewall.oneToOneNat,
  )
  ...
  const handleDelete = (key: string) => {
    sendWsSetMessage(XPB_EVENT_ACTIONS.XPB_510_FIREWALL_DELETE_ONE_TO_ONE_NAT, {
      'cbi.submit': '1',
  ...
  useEffect(() => {
    setIsFetch(true)
    sendWsGetMessage(XPB_EVENT_ACTIONS.XPB_510_FIREWALL_GET_ONE_TO_ONE_NAT_PAGE)
    ...
    <>
      ...
      {!data ? (
        ...
        <Stack gap={2}>
          ...
          <AddOneToOneNatForm suggest={suggest} options={options} list={list} />
        </Stack>
      )}
