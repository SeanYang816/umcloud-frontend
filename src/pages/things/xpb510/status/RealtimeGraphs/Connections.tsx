...
import { Connection } from 'types/xpb510/status/RealTimeGraphs'
...
export const Connections = () => {
  ...
  const data = useSelector(
    (state: RootStateProps) => state.xpb510.status.realtimeGraphs.connections,
  )
  // const statistics = result?.statistics || []
  const connections: Connection[] = data?.result?.connections ?? []

  const initialArr = Array(240).fill(0)
  const [udp, _setUdp] = useState<number[]>(initialArr)
  const [tcp, _setTcp] = useState<number[]>(initialArr)
  const [other, _setOther] = useState<number[]>(initialArr)
  ...
  useEffect(() => {
    const sendMessage = () => {
      sendWsGetMessage(XPB_EVENT_ACTIONS.XPB_510_REAL_TIME_GRAPHS_CONNECTIONS)
    }
