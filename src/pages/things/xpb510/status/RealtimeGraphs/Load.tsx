...
import { resetRealTimeGraphsState } from 'reducers/xpb510/status/realtimeGraphs'
...
export const Load = () => {
  ...
  const data = useSelector(
    (state: RootStateProps) => state.xpb510.status.realtimeGraphs.load,
  )
  ...
  useEffect(() => {
    if (!result || !result.length || result.length === 1) return
    if (!isStart) {
      setIsStart(true)
  ...
  useEffect(() => {
    const sendMessage = () => {
      sendWsGetMessage(XPB_EVENT_ACTIONS.XPB_510_REAL_TIME_GRAPHS_LOAD)
    }
    intervalRef.current = setInterval(sendMessage, 3000) as unknown as number

    return () => {
      clearInterval(intervalRef.current as unknown as number)
      dispatch(resetRealTimeGraphsState())
    }
